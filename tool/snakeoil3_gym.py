...
class ServerState():
    ...
    def fancyout(self):
        ...
        out= str()
        sensors= [ # Select the ones you want in the order you want them. focus[0],...,focus[5]
        #'curLapTime',      # the time elapsed during the current lap and the unit is in second(Seconds since current lap began)
        #'lastLapTime',     # measure the time that used to complete the last lap(Seconds it took to do last lap. Usually 0 for most updates.)
        'stucktimer',
        #'damage',          # record the damage level of the racing car by points
        #'focus',           # adjust the direction of the focus sensor(from -90 angle~90 angle)  the original setting of the focus is 0
        'fuel',             # fuel level of the vehicle in the simulation    maximun value is 100 in the simulation
        #'gear',            # current gear of the racing car (齿轮 变速器)  from -1 to 7: -1 means reverse, 0 is neutral and gear is from 1 and up to 7
        'distRaced',        # record the distance covered by the car from the beginning of the race    distRaced + distFromStart = total length of the lap
        'distFromStart',    # record the distance of the car from the start line along the track line
        #'racePos',         # measure the current position during the race respect to other racing cars on the same track(Current place in race)
        'opponents',        # 对手,敌手,反对者  opponents function has vector of 36 opponents sensors and same with track function, each sensor cover a span of 10 degrees within a range of 200 meters    dist to nearest opponent
        'wheelSpinVel',     # stand for spin velocity(自转角速度)  This function has four sensors to measure the rolation speed of each wheel to let us easier to analyze the data of the racing car after the race
        'z',                # the distance of the car mass centre from the surface of the track along the Z axis(汽车的重心沿Z轴距离轨道表面的距离)
        'speedZ',           # the speed of the racing car along the Z axis of the racing car(单位: km/h)
        'speedY',           # the speed of the racing car along the transverse axis(水平轴,横轴) of the racing car
        'speedX',           # the speed of the racing car along the longitudinal axis(纵向轴) of the racing car
        'targetSpeed',
        'rpm',              # this is the number of rotation(旋转,循环,转动) per minute of the car engine(Current engine speed)(单位: rpm =round per minute 每分钟多少转)
        'skid',             # A sensible interpretation of wheel spin.
        'slip',             # A sensible interpretation of wheel spin
        'track',            # This function is measure the distance between the front of the racing car and the coming track edge(has 19 ranges of vector finder sensors, the range of each sensor is up to 200 meters. The vector of each sensor has 10 degrees different, spanning clockwise(顺时针方向) from -90 degree to +90 degree)
        'trackPos',         # means the track position of the racing car on the track(means the distance between the racing car and the track axis, 0 is when the car is on the axis, -1 when the racing car is on the right edge of the track and +1 is when the racing car is on the left edge of the track  -1 mean car is off track)    The width of the track is 15 meters
        'angle',            # the angle between the running direction of the vehicle and the direction of the track axis
        ]
        ...
        for k in sensors:
            if type(self.d.get(k)) is list: # Handle list type data.
                ...
                if k == 'gear': # This is redundant(多余的) now since it's part of RPM.
...
class DriverAction():
    ...
    def __init__(self):
       ...
       self.d= { 'accel':0.2,                       # This accelerates the car based upon a value of 0 or 1 which are its minimum and maximum acceleration respectively(Virtual gas pedal: 0 means no gas, 1 full gas)
                   'brake':0,                       # The maximum to minimum braking is applied for a value between 0 and 1(Virtual brake pedal: 0 means no brake, 1 full brake)
                  'clutch':0,                       # Virtual clutch pedal(离合器踏板 0 means no clutch, 1 full clutch)
                    'gear':1,                       # -1~7(包含头不包含尾)
                   'steer':0,                       # the steering value    -1 and +1 mean respectively full right and left
                   'focus':[-90,-45,0,45,90],       # focus direction
                    'meta':0                        # This is meta-control command: 0 do nothing, 1 ask competition server to restart the race
                    }
...
def drive_example(c):
    ...
    # Traction Control System(牵引控制)
    if ((S['wheelSpinVel'][2]+S['wheelSpinVel'][3]) -
       (S['wheelSpinVel'][0]+S['wheelSpinVel'][1]) > 5):
       R['accel']-= .2

    # Automatic Transmission(自动驾驶)
    R['gear']=1
    if S['speedX']>50:
        R['gear']=2
    if S['speedX']>80:
